function [score] = local_score_CV_multi(Data,Xi,PAi,parameters) % calculate the local score
...
%     theta = 1/(width^2);
    ...
    for m = 1:length(PAi)
        ...
        width = sqrt(0.5*median(dists(dists>0))); % median value
        width = width*3; %%%
        theta = 1/(width^2*size(PA,2)); % 
%         theta = 1/(width^2);
        Kpa = Kpa.*kernel(PA, PA, [theta,1]);
    end
    H0 =  eye(T) - ones(T,T)/T; % for centering of the data in feature space
